...
function writeActivityLog(action, payload, result) {
  try {
    ...
    row[hdr["PayloadSummary"]] = JSON.stringify(payload || {}).substring(
      0,
      500,
    );
    ...
    try {
      Logger.log("[writeActivityLog] failed: " + e.message);
    } catch (e2) {
      /* truly never throw */
    }
  }
  ...
  try {
    switch (action) {
      ...
      case "deactivateCustomer":
        return deactivateCustomer(payload);
      case "getCustomers":
        return getCustomers(payload);
      case "addPausePeriod":
        return addPausePeriod(payload);
      case "updateLogEntry":
        return updateLogEntry(payload);
      case "bulkUpsertLogs":
        return bulkUpsertLogs(payload);
      case "getDailyLogs":
        return getDailyLogs(payload);
      case "generateMonthBill":
        return generateMonthBill(payload);
      case "updateBill":
        return updateBill(payload);
      case "finalizeBill":
        return finalizeBill(payload);
      case "lockBill":
        return lockBill(payload);
      case "unlockBill":
        return unlockBill(payload);
      case "recordPayment":
        return recordPayment(payload);
      case "addAdjustment":
        return addAdjustment(payload);
      case "applyAdjustment":
        return applyAdjustment(payload);
      case "getBills":
        return getBills(payload);
      case "getBillText":
        return getBillText(payload);
      case "reconcileBillingLedger":
        return reconcileBillingLedger(payload);
      case "addMilkImport":
        return addMilkImport(payload);
      case "updateMilkImport":
        return updateMilkImport(payload);
      case "confirmMilkImport":
        return confirmMilkImport(payload);
      case "deleteMilkImport":
        return deleteMilkImport(payload);
      case "getMilkImports":
        return getMilkImports(payload);
      case "getMilkImportSummary":
      ...
      case "addMilkBrand":
        return addMilkBrand(payload);
      case "getMilkBrands":
        return getMilkBrands(payload);
      case "getMilkTypes":
        return getMilkTypes(payload);
      case "verifyPIN":
        return verifyPIN(payload);
      case "rotatePIN":
        return rotatePIN(payload);
      case "batchFlush":
        return batchFlush(payload);
      case "healthCheck":
        return healthCheck();
      case "runDiagnostics":
        return runDiagnostics();
      case "getSheetNamesAction":
        return getSheetNamesAction();
      case "eraseAllData":
